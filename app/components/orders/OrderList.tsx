    ...
    <div className={styles.container}>
      <div className={styles.ordersList}>
         {orders.map((order) => {
          ...
          const [nombre, email, telefono] = order.usuario_id === 1 && order.notas
          ? order.notas.split("-*-")
          : [order.usuarios.nombre, order.usuarios.email, order.usuarios.telefono ?? ""];

            const clienteNombre = nombre || "Sin nombre";
            const clienteEmail = email || "Sin email";
            const clienteTelefono = telefono || "Sin teléfono";
              ...
              <div className={styles.orderHeader} onClick={() => toggleOrder(order.id)}>
                ...
                <div className={styles.orderSummary}>
                  <div className={styles.customerInfo}>
                    <span className={styles.customerName}>{clienteNombre}</span>
                    <span className={styles.customerEmail}>{clienteEmail}</span>
                  </div>
